...
| `TELEGRAM_REACTION_COMMANDS` | Enable incoming reaction commands — user reactions on existing messages map to deterministic actions (default map: 👎 → `/stop`). Overrides `telegram.reaction_commands.enabled`. Default: `false`. |
| `TELEGRAM_QUICK_KEYBOARD` | Enable the persistent quick keyboard — always-visible reply-keyboard buttons whose labels dispatch mapped commands/prompts deterministically (attach with `/keyboard`). Overrides `telegram.quick_keyboard.enabled`. Default: `false`. |
| `TELEGRAM_QUICK_ACTIONS` | Enable quick-action buttons — an inline row attached under the final response after delivery (e.g. 🔁 Retry, ⏹ Stop). Overrides `telegram.quick_actions.enabled`. Default: `false`. |
