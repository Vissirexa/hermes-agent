...
def clear_task_trackers(task_id: str) -> None:
    ...
    ``_read_tracker``, ``_patch_failure_tracker``, the terminal_tool cwd
    registry and ``_file_ops_cache`` are keyed by task_id and nothing else removes a
    ...
    *within* one entry. Subagent task ids are unique per delegation
    ...
    # The per-task cwd record moved to terminal_tool's session registry;
    # clear_session_cwd pops the exact key, matching the alias contract above.
    try:
